...
def centroid_to_zone(cx, cy, zones):
...
history = defaultdict(lambda: deque(maxlen=required))
confirmed = {}
    ...
    try:
        ...
        if retval and decoded_info is not None:
            for info, pts in zip(decoded_info, points):
                ...
                logger.info('Detected payload=%s centroid=(%d,%d) zone=%s history=%s', info, cx, cy, list(history[info]))
                if len(history[info]) == required and len(set(history[info])) == 1:
