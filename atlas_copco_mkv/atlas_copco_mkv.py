...
try:
    ...
    def post_question(host: str, qhex: str, timeout_sec: int) -> str:
        ...
        resp = requests.post(uri, data={"QUESTION": qhex}, headers={"Content-Type":"application/x-www-form-urlencoded"}, timeout=timeout_sec)
    ...
    def post_question(host: str, qhex: str, timeout_sec: int) -> str:
        ...
        req = urllib.request.Request(uri, data=data, headers={"Content-Type":"application/x-www-form-urlencoded"}, method="POST")
...
def hex_slice(hexstr: str, offset: int, length: int) -> str:
    if offset < 0 or offset + length > len(hexstr): return ""
    return hexstr[offset:offset+length].upper()

def hex_to_uint32_be(hex8: str) -> Optional[int]:
    if not hex8 or len(hex8) != 8 or not re.match(r"^[0-9A-F]{8}$", hex8): return None
    return int(hex8, 16)
...
def resolve_external_refs(expr: str, key_to_u32: Dict[str, Optional[int]], key_to_lo: Dict[str, Optional[int]], key_to_hi: Dict[str, Optional[int]]) -> Tuple[str, bool]:
    ok = True
    def sub_generic(m: re.Match, d: Dict[str, Optional[int]]) -> str:
        ...
        if val is None:
            ...
            return ""
        return str(val)
    expr = re.sub(r"UInt32of([0-9A-F]{4})\.([0-9A-F]{2})", lambda m: sub_generic(m, key_to_u32), expr)
    expr = re.sub(r"LoU16of([0-9A-F]{4})\.([0-9A-F]{2})",  lambda m: sub_generic(m, key_to_lo),  expr)
    expr = re.sub(r"HiU16of([0-9A-F]{4})\.([0-9A-F]{2})",  lambda m: sub_generic(m, key_to_hi),  expr)
    return expr, ok

def eval_calc(calc: str, u32: Optional[int], lo: Optional[int], hi: Optional[int],
              key_to_u32: Dict[str, Optional[int]], key_to_lo: Dict[str, Optional[int]], key_to_hi: Dict[str, Optional[int]]) -> Optional[float]:
    if not calc or calc.strip() == "?": return None
    expr = calc
    expr = re.sub(r"UInt32", str(u32) if u32 is not None else "", expr)
    expr = re.sub(r"LoU16",  str(lo) if lo is not None else "", expr)
    expr = re.sub(r"HiU16",  str(hi) if hi is not None else "", expr)
...
[2] Custom")
    while True:
        ...
        if sel in {"0","1","2"}: break
    return ["GA15VS23A","GA15VP13","Custom"][int(sel)]

def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Atlas Copco MK5s Touch poller (Python port).",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=textwrap.dedent("Host auto-selection (unless overridden with --controller-host):
  ...
  - GA15VS23A -> 10.60.23.12"))
    ...
    if qset in ("GA15VS23A","GA15VP13"):
        ...
        if not qh:
            print("No custom question hex provided.", file=sys.stderr); return 2
        question_hex = qh
    else:
        print(f"Unknown QuestionSet: {qset}", file=sys.stderr); return 2

    host = args.controller_host
    if host is None:
        host = "10.60.23.11" if qset=="GA15VP13" else ("10.60.23.12" if qset=="GA15VS23A" else None)
    if host is None:
        print("Error: --controller-host is required for Custom question set.", file=sys.stderr); return 2
    ...
    for idx, k in enumerate(keys):
        ...
        for meta_entry in metas:
            ...
            if val is not None:
                ...
                val_out = None
            rows.append({
                "Device": device_name, "Type": device_type, "Key": key,
                "Name": meta_entry.get("Name"), "Raw": raw,
                "UInt32": u32, "LoU16": lo, "HiU16": hi,
                "Encoding": meta_entry.get("Encoding"), "Calc": calc,
                "Value": val_out, "Unit": meta_entry.get("Unit"),
            })
