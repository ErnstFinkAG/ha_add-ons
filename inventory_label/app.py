...
UI_STRINGS = {
    "en": {
        ...
        "effective_print_width": "Effective print width at the configured printer DPI",
    ...
    "de": {
        ...
        "effective_print_width": "Effektive Druckbreite mit der konfigurierten Drucker-DPI",
...
def normalize_printer_dpi(value: object, default: int = DEFAULT_PRINTER_DPI) -> int:
    return normalize_int(value, default, 100, 1200)
...
def normalize_profile(raw: object, idx: int) -> Dict:
    ...
    return {
        ...
        "print_offset_x_mm": normalize_float(data.get("print_offset_x_mm"), 0.0, -100.0, 100.0),
        "print_offset_y_mm": normalize_float(data.get("print_offset_y_mm"), 0.0, -100.0, 100.0),
...
def dots_per_mm(profile: Dict | None = None) -> float:
    return normalize_printer_dpi((profile or {}).get("printer_dpi"), DEFAULT_PRINTER_DPI) / 25.4
...
def mm_to_dot_offset(mm_value: float, profile: Dict | None = None) -> int:
...
def apply_profile_print_offsets(img: Image.Image, profile: Dict | None = None) -> Image.Image:
    ...
    copy_w = min(img.width - src_x, img.width - dst_x)
    copy_h = min(img.height - src_y, img.height - dst_y)
    if copy_w > 0 and copy_h > 0:
        region = img.crop((src_x, src_y, src_x + copy_w, src_y + copy_h))
...
def printable_area_box_margin_mm(profile: Dict | None = None) -> float:
...
def get_printable_area_rect(
    ...
    oriented_width_mm, oriented_height_mm = oriented_label_size_mm(profile, rotation_degrees)
    margin_mm = printable_area_box_margin_mm(profile)
    box_width_mm = max(0.0, oriented_width_mm - (2.0 * margin_mm))
    box_height_mm = max(0.0, oriented_height_mm - (2.0 * margin_mm))

    scale_x = width / max(oriented_width_mm, 0.001)
    scale_y = height / max(oriented_height_mm, 0.001)

    left = int(round(margin_mm * scale_x))
    top = int(round(margin_mm * scale_y))
    right = int(round((margin_mm + box_width_mm) * scale_x)) - 1
    bottom = int(round((margin_mm + box_height_mm) * scale_y)) - 1
...
def draw_printable_area_box(
    ...
    if not normalize_bool((profile or {}).get("printable_area_box_enabled"), False):
        return
    left, top, right, bottom = get_printable_area_rect(
        img.width,
        img.height,
        profile,
...
def render_portrait_content(printable_w: int, canvas_h: int, qr_value: str, body_blocks: List[Dict], footer_blocks: List[Dict], profile: Dict, preview: bool) -> Image.Image:
    ...
    if has_qr:
        ...
        qr_left = area_left + max((area_width - qr_size) // 2, 0)
        qr_top = current_y
...
def render_rotated_content(printable_w: int, canvas_h: int, qr_value: str, body_blocks: List[Dict], footer_blocks: List[Dict], profile: Dict, preview: bool, rotation_degrees: int) -> Image.Image:
    ...
    has_qr = bool(normalize_qr_value(qr_value))
    area_left, area_top, area_right, area_bottom = get_printable_area_rect(logical_w, logical_h, profile, rotation_degrees=90)
    area_width = max(1, area_right - area_left + 1)
    ...
    if has_qr:
        ...
        qr_left = min(max(area_left + layout["top_margin_dots"], area_left), max(area_left, area_right - qr_size + 1))
        qr_top = area_top + max((area_height - qr_size) // 2, 0)
        qr_img = build_qr_image(qr_value, qr_size, profile).convert("RGB")
...
def render_label_image(qr_value: str, field_forms: List[Dict], profile: Dict, preview: bool) -> Image.Image:
    ...
    if not preview:
        ...
        draw_printable_area_box(printable_image, profile, preview=False, apply_offsets=True, rotation_degrees=0)
        return printable_image
    if requested_w <= printable_w:
        preview_image = orient_preview_for_display(printable_image, rotation_degrees)
        draw_printable_area_box(preview_image, profile, preview=True, apply_offsets=False, rotation_degrees=rotation_degrees)
    ...
    preview_image = orient_preview_for_display(canvas, rotation_degrees)
    draw_printable_area_box(preview_image, profile, preview=True, apply_offsets=False, rotation_degrees=rotation_degrees)
    return preview_image
