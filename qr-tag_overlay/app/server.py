...
async def fetch_camera_snapshot(session: ClientSession, entity_id: str) -> bytes:
    url = f"{CORE_API}/camera_proxy/{entity_id}"
    headers = {"Authorization": f"Bearer {SUPERVISOR_TOKEN}"}
    async with session.get(url, headers=headers) as resp:
        if resp.status != 200:
            body = await resp.text()
            raise RuntimeError(f"Camera proxy failed: {resp.status} {body[:200]}")
...
async def updater(state: State, entity_id: str, poll_ms: int, overlay_cfg: dict):
    timeout = ClientTimeout(total=10)
    async with ClientSession(timeout=timeout) as session:
        while True:
            try:
                raw = await fetch_camera_snapshot(session, entity_id)
...
def main():
    ...
    loop.create_task(updater(state, entity_id, poll_ms, overlay_cfg))
